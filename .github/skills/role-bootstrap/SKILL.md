...
### Step 0: 目錄瀏覽與路徑選擇 [interactive]

讀取 `catalog/archetypes/_index.yaml`，以目錄卡片格式展示所有可用原型（編號、icon、name、description、技能列表）。
...
跳過 Step 1 和 Step 2，直接進入 Step 3 的 Seed 複製路徑。
複製時只替換 `identity.yaml` 中的 `role.name` 和 `role.alias`，其餘維持原型預設值。

**路徑 (b) 以原型微調**

預載已選原型的預設值，告知使用者：「已載入 [原型名稱] 原型，以下為預設值，訪談將聚焦在需要客製的部分。」
Step 1a 改為直接展示預設值（無需自動匹配）；Step 1b 只問 `discovery_questions` 中的 differentiation + principles 兩組（跳過 context 組），約 6 個問題。
...
> 路徑 (a) 跳過此步驟。路徑 (b) 使用精簡版。路徑 (c) 執行完整版。

**1a. Archetype 預設值載入**

- 路徑 (b)：直接展示已在 Step 0 選定的原型預設值
- 路徑 (c)：讀取 `catalog/archetypes/_index.yaml`，根據使用者描述自動匹配；
  - 若有匹配 → 告知使用者「已匹配到 [原型名稱] 原型，以下預設值可直接採用或調整」
  - 若無匹配 → 告知使用者「未找到匹配的角色原型，將從空白開始訪談」
  - 若有部分匹配 → 列出候選原型，讓使用者選擇
...
若使用了 archetype，參照 `catalog/archetypes/` 對應的 `sample-principles.md` 作為品質基準。
...
先檢查 `catalog/archetypes/_index.yaml` 中匹配的 archetype 是否有 `has_prebuilt: true`：
...
1. `.ai/identity.yaml` — 角色靈魂，最重要（seed 或直接產生）
2. `.ai/principles.md` — 判斷框架（seed 或直接產生）
3. `.ai/knowledge/glossary.yaml` — 角色專屬術語（seed 或直接產生，至少 8 條）
4. `.ai/interfaces/exports.yaml` + `imports.yaml` — 介面定義（seed 或直接產生）
5. `AGENTS.md` — 核心指令源（依照本 Repo AGENTS.md 的結構，填入客製值直接產生）
6. `CLAUDE.md` / `GEMINI.md` / `.github/copilot-instructions.md` — 三個橋接檔
   - `CLAUDE.md`：只含標題行 + `@AGENTS.md`（用 @ 語法引入，不寫散文說明）
   - `GEMINI.md`：只含 `@./AGENTS.md`（Gemini CLI @ 語法）
   - `.github/copilot-instructions.md`：精簡橋接內容（≤ 60 行，詳見 copilot-sync Skill）
7. `.github/skills/_index.yaml` — 合併 archetype skills + default skills
8. 角色專屬技能（從 archetype `skills/` 複製，或依照 SKILL 規範直接產生）
9. **預設技能**（自動包含，不需使用者設定）：
   讀取 RoleFoundry 的 `catalog/default-skills/_defaults.yaml`，
   將其中所有預設技能從 `catalog/default-skills/` 複製到目標 Repo 的 `.github/skills/`，
...
- 建議後續：執行 `collab-join` Skill 將此角色加入協作空間（5 分鐘完成）
