...
讀取 `context/archetypes/_index.yaml`，嘗試匹配最接近的原型：
...
在 `.github/agents/` 下建立 `{role-b-alias}.agent.md`，使用 `templates/agent.md.tmpl` 模板。

內容包含：
- **Role**：角色名稱、alias、one_liner
- **Responsibilities**：核心職責（從訪談結果萃取）
- **Constraints**：能力邊界（owns / advises / defers_to）
- **Relationship**：與主角色的協作關係（雙向 defer 說明）
...
- 若匹配到 archetype 且有 `has_prebuilt: true`：
  從 `context/archetypes/{alias}/skills/` 複製對應技能
- 若無匹配：
  使用 `templates/SKILL.md.tmpl` 產生空白技能結構
